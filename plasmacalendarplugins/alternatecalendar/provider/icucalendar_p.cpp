...
{
    m_GregorianCalendar.reset(icu::Calendar::createInstance("en_US@calendar=gregorian", m_errorCode));
}
...
{
    int32_t year = m_calendar->get(UCAL_YEAR, m_errorCode);
...
{
    int32_t month = m_calendar->get(UCAL_MONTH, m_errorCode);
...
{
    int32_t day = m_calendar->get(UCAL_DATE, m_errorCode);
...
bool ICUCalendarPrivate::setDate(const QDate &date)
{
    ...
    UDate time = m_GregorianCalendar->getTime(m_errorCode);
