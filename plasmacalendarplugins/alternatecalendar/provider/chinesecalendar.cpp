...
constexpr static std::array<MoonEclipticLongitudeCoeff, 60> s_moonLongitude{
...
constexpr static std::array<NuationCoefficient, 63> s_nuation{
...
static const QStringList s_solarTermNames{
 ...
 */
double getSunEclipticLongitudeForEarth(double julianDay)
{
    double t = getJulianThousandYears(julianDay);
...
{
    // days of 100 years
    constexpr double DaysOfCentury = 36525.0;
 ...
 */
double Vsop2Fk5LongitudeCorrection(double latitude, double longitude, double julianDay)
{
    double t = getJulianCentury(julianDay);
    double lp = latitude - degreesToRadians(1.397) * t - degreesToRadians(0.00031) * t * t;
    return secondsToRadians(-0.09033 + 0.03916 * (std::cos(lp) + std::sin(lp)) * std::tan(longitude));
}

/**
 * Calcuates the distance between sun and earth

 * @return distance in au
 */
double getSunRadiusForEarth(double julianDay)
{
    double t = getJulianThousandYears(julianDay);
...
double getEarthEclipticLongitudeForSun(double jd)
{
    ...
    double b = getSunEclipticLongitudeForEarth(jd);
...
{
    double A1 = mod2Pi(degreesToRadians(119.75 + 131.849 * T));
    double A2 = mod2Pi(degreesToRadians(53.09 + 479264.290 * T));
...
double getMoonEclipticLongitudeEC(double julianDay)
{
    ...
    double longitude = m_radian.Lp + degreesToRadians(EI / 1000000.0);
...
void getDateFromJulianDay(double julianDay, int &yy, int &mm, int &dd)
{
     ...
     */
    constexpr int JD_GREG_CAL = 2299161;
    constexpr int JB_MAX_WITHOUT_OVERFLOW = 107374182;
...
CalendarEvents::CalendarEventsPlugin::SubLabel ChineseCalendarProviderPrivate::subLabels(const QDate &date)
{
    ...
    sublabel.dayLabel = day() == 1 ? monthDisplayName() : (solarTermIndex >= 0 ? SolarUtils::s_solarTermNames.at(solarTermIndex) : dayName);
    const QString solarTerm = solarTermIndex >= 0 ? QStringLiteral(" (%1)").arg(SolarUtils::s_solarTermNames.at(solarTermIndex)) : QString();
...
QDate ChineseCalendarProviderPrivate::getSolarTermDate(int year, SolarTerm order) const
{
    ...
    double julianDate = SolarUtils::toJulianDateHMS(year, month, day, 12, 0, 0.0);
    double julianDay = SolarUtils::NewtonIteration(angle, julianDate);
    // To Beijing time
    julianDay += 8.0 / 24.0;
