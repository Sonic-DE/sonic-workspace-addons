...
void PotdClient::slotFinished(PotdProvider *provider, const QImage &image)
{
    ...
    if (qobject_cast<CachedProvider *>(provider) == nullptr) {
        QVariantMap data;
        data.insert(QStringLiteral("RemoteUrl"), provider->remoteUrl());
        data.insert(QStringLiteral("InfoUrl"), provider->infoUrl());
        data.insert(QStringLiteral("Title"), provider->title());
        data.insert(QStringLiteral("Author"), provider->author());
        data.insert(QStringLiteral("RemoteUrl"), provider->remoteUrl());
