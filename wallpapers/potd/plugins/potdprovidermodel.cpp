...
void PotdProviderModel::saveImage()
{
    ...
    // clang-format off
    QString defaultFileName = m_providers.at(m_currentIndex).name();

    if (!m_data.wallpaperTitle.isEmpty()) {
        defaultFileName += QLatin1Char('-') + m_data.wallpaperTitle;
        if (!m_data.wallpaperAuthor.isEmpty()) {
            defaultFileName += QLatin1Char('-') + m_data.wallpaperAuthor;
        }
    } else {
        // Use current date
        defaultFileName = (defaultFileName.isEmpty() ? QLatin1Char('-') : QChar()) + QDate::currentDate().toString();
    }
