...
void Kameleon::setEnabled(bool enabled)
{
    if (enabled != m_enabled) {
        ...
        config->group("General").writeEntry<bool>("AccentColoredLedDevices", enabled);
...
{
    qCInfo(KAMELEON) << "load config";

    KSharedConfig::Ptr config = KSharedConfig::openConfig("kdeglobals");

    m_enabled = config->group("General").readEntry<bool>("AccentColoredLedDevices", true);
