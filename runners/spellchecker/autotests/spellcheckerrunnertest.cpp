...
void SpellCheckRunnerTest::testDefaultDictionary()
{
    ...
    QVERIFY(matches.count() == 1);
    QVERIFY(matches.constFirst().text() == QStringLiteral("hello") && matches.constFirst().iconName() == QStringLiteral("checkbox"));
}
...
void SpellCheckRunnerTest::testSpecifiedDictionary()
{
    ...
    if (m_hasRUDictionary) {
        ...
        auto matches = context.matches();
        QVERIFY(matches.count() == 1);
        QVERIFY(matches.constFirst().text() == QStringLiteral("мама") && matches.constFirst().iconName() == QStringLiteral("checkbox"));
    }
}
...
void SpellCheckRunnerTest::testAutomaticDictionary()
{
    ...
    const auto matches = context.matches();
    QVERIFY(matches.count() == 1);
    QVERIFY(matches.constFirst().text() == QStringLiteral("мама") && matches.constFirst().iconName() == QLatin1String("checkbox"));
}
...
#include "spellcheckerrunnertest.moc"
