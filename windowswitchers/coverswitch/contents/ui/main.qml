/*
 SPDX-FileCopyrightText: 2021-2022 Ismael Asensio <isma.af@gmail.com>
...
KWin.TabBoxSwitcher {
    ...
    property int currentDesktop: KWin.Workspace.currentDesktop
    ...
    hidingDelay: PlasmaCore.Units.veryLongDuration
    ...
    Window {
        ...
        Rectangle {
            ...
            opacity: enableBlur ? (originalState ? 0 : 0.5) : 0.75
        ...
        PathView {
            ...
            delegate: Item {
                ...
                readonly property var desktopIds: thumbnail.client ? thumbnail.client.x11DesktopIds : []
                ...
                state: {
                    ...
                    if (desktopIds.length === 0 || desktopIds.includes(tabBox.currentDesktop)) return "clientPosition";
                    return "hidden";
                }
            ...
            highlight: PlasmaCore.FrameSvgItem {
                ...
                width: target ? Math.round(target.width * target.scaleFactor / 2 + 3 * PlasmaCore.Units.smallSpacing) * 2 : 0
                height: target ? Math.round(target.height * target.scaleFactor / 2 + 3 * PlasmaCore.Units.smallSpacing) * 2 : 0
    ...
    onAboutToHide: {
        if (thumbnailView.currentItem
                && thumbnailView.currentItem.desktopIds.length > 0  // not on all desktops
                && !thumbnailView.currentItem.desktopIds.includes(tabBox.currentDesktop)) {
            // Select the latest VD the window is on, to mimic Workspace.activate()
            tabBox.currentDesktop = thumbnailView.currentItem.desktopIds.slice(-1)[0]
        }
        thumbnailView.positionViewAtIndex(tabBox.currentIndex, PathView.SnapPosition)
        originalState = true
    }
