...
DropArea {
    ...
    Layout.minimumWidth: root.isVertical ? PlasmaCore.Units.iconSizes.small : (grid.height * 2 + spacer.width)
    Layout.minimumHeight: root.isVertical ? (grid.width * 2 + spacer.height) : PlasmaCore.Units.iconSizes.small

    readonly property real buttonSize: root.isVertical ? dropArea.parent.width : dropArea.parent.height
