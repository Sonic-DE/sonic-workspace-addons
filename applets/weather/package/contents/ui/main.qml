...
Item {
    ...
    property int sourceStatus: Util.Normal
    ...
    PlasmaCore.DataSource {
        ...
        onConnectedSourcesChanged: {
            if (weatherSource) {
                sourceStatus = Util.Connecting
                connectionTimeoutTimer.start();
            }
        }
        onCurrentDataChanged: {
            if (currentData) {
                sourceStatus = Util.Normal
    ...
    Timer {
        ...
        repeat: false
        onTriggered: {
            sourceStatus = Util.Timeout;
        }
    }

    // workaround for now to ensure "Please configure" tooltip
    // TODO: remove when configurationRequired works
    Plasmoid.icon: (sourceStatus === Util.NeedsConfiguration) ? "configure" : generalModel.currentConditionIconName
    Plasmoid.busy: sourceStatus === Util.Connecting
    Plasmoid.configurationRequired: sourceStatus === Util.NeedsConfiguration

    Plasmoid.toolTipMainText: (sourceStatus === Util.NeedsConfiguration) ?
    ...
    onWeatherSourceChanged: {
        if (weatherSource.length === 0) {
            sourceStatus = Util.NeedsConfiguration
        }
    }
