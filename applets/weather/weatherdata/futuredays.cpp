...
QString FutureDays::firstDayIcon() const
{
    ...
    if (const auto &daytime = dayForecast.daytime(); daytime.has_value()) {
        if (const auto &conditionIcon = daytime->conditionIcon(); conditionIcon.has_value()) {
    ...
    if (const auto &night = dayForecast.night(); night.has_value()) {
        if (const auto &conditionIcon = night->conditionIcon(); conditionIcon.has_value()) {
            return *conditionIcon;
        }
        return {};
    }
