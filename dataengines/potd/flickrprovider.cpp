...
void FlickrProvider::keyRequestFinished(KJob *_job)
{
    ...
    const QUrl jsonUrl = buildUrl(mActualDate, mApiKey);

    KIO::StoredTransferJob *jsonJob = KIO::storedGet(jsonUrl, KIO::NoReload, KIO::HideProgressInfo);
    connect(jsonJob, &KIO::StoredTransferJob::finished, this, &FlickrProvider::jsonRequestFinished);
}
...
void FlickrProvider::jsonRequestFinished(KJob *_job)
{
            ...
            if (xml.name() == QLatin1String("rsp")) {
                ...
                if (attributes.value ( QLatin1String( "stat" ) ).toString() != QLatin1String( "ok" )) {
                    if (mFailureNumber < maxFailure) {
                        ...
                        connect(pageJob, &KIO::StoredTransferJob::finished, this, &FlickrProvider::jsonRequestFinished);
                        mFailureNumber++;
                        return;
                    } else {
                        emit error(this);
