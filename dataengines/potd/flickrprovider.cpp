...
void FlickrProvider::xmlRequestFinished(KJob *_job)
{
    ...
    if (m_photoList.begin() != m_photoList.end()) {
        const auto randomNumber = QRandomGenerator::global()->bounded(static_cast<unsigned>(m_photoList.size()));
        const auto &randomPhotoEntry = m_photoList.at(randomNumber);
        m_data->wallpaperRemoteUrl = QUrl(randomPhotoEntry.urlString);
        m_data->wallpaperTitle = randomPhotoEntry.title;
        ...
        if (!(randomPhotoEntry.userId.isEmpty() || randomPhotoEntry.photoId.isEmpty())) {
            m_data->wallpaperInfoUrl = QUrl(QStringLiteral("https://www.flickr.com/photos/%1/%2").arg(randomPhotoEntry.userId).arg(randomPhotoEntry.photoId));
        }
...
void FlickrProvider::imageRequestFinished(KJob *_job)
{
    ...
    // Visit the photo page to get the author
    if (!m_data->wallpaperInfoUrl.isEmpty()) {
        KIO::StoredTransferJob *pageJob = KIO::storedGet(m_data->wallpaperInfoUrl, KIO::NoReload, KIO::HideProgressInfo);
        connect(pageJob, &KIO::StoredTransferJob::finished, this, &FlickrProvider::pageRequestFinished);
    }

    m_data->wallpaperImage = QImage::fromData(job->data());
}
...
void FlickrProvider::pageRequestFinished(KJob *_job)
{
    ...
    if (match.hasMatch()) {
        m_data->wallpaperAuthor = QTextDocumentFragment::fromHtml(match.captured(1).trimmed()).toPlainText();
    }
