...
void EpodProvider::pageRequestFinished(KJob *_job)
{
    ...
    if (exp.indexIn(data) != -1) {
        ...
        if (titleMatch.hasMatch()) {
            m_wallpaperInfoUrl = QUrl(titleMatch.captured(1).trimmed());
            m_wallpaperTitle = QTextDocumentFragment::fromHtml(titleMatch.captured(2).trimmed()).toPlainText();
        }
        ...
        if (authorMatch.hasMatch()) {
            m_wallpaperAuthor = QTextDocumentFragment::fromHtml(authorMatch.captured(1).trimmed()).toPlainText();
        }
    } else {
        Q_EMIT error(this);
        return;
    ...
    m_wallpaperRemoteUrl = QUrl(QStringLiteral("https://epod.usra.edu/.a/%1-pi").arg(sub));
