...
void NOAAProvider::listPageRequestFinished(KJob *_job)
{
    ...
    if (result.hasMatch()) {
        m_data->wallpaperInfoUrl = QUrl(QStringLiteral("https://www.nesdis.noaa.gov") + result.captured(1));
    }
    if (!m_data->wallpaperInfoUrl.isValid()) {
...
void NOAAProvider::pageRequestFinished(KJob *_job)
{
    ...
    if (result.hasMatch()) {
        m_data->wallpaperRemoteUrl = QUrl(QStringLiteral("https://www.nesdis.noaa.gov") + result.captured(1));
    }
    if (!m_data->wallpaperRemoteUrl.isValid()) {
    ...
    if (titleMatch.hasMatch()) {
        m_data->wallpaperTitle = QTextDocumentFragment::fromHtml(titleMatch.captured(1).trimmed()).toPlainText();
    }
