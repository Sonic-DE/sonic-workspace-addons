...
void BingProvider::pageRequestFinished(KJob *_job)
{
    ...
    do {
        ...
        if (const QRegularExpressionMatch match = copyrightRegEx.match(copyright); match.hasMatch()) {
            ...
            m_data->wallpaperTitle = match.captured(1).trimmed();
            m_data->wallpaperAuthor = match.captured(2).remove(QStringLiteral("©")).trimmed();
        }
        ...
        if (!infoUrl.isEmpty()) {
            m_data->wallpaperInfoUrl = QUrl(infoUrl);
        }

        m_data->wallpaperRemoteUrl = QUrl(QStringLiteral("https://www.bing.com/%1").arg(url.toString()));
