...
void WcpotdProvider::pageRequestFinished(KJob *_job)
{
    ...
    if (!imageFile.isEmpty()) {
        m_data->wallpaperRemoteUrl = QUrl(QLatin1String("https://commons.wikimedia.org/wiki/Special:FilePath/") + imageFile);
    ...
    const QJsonObject jsonTextObject = jsonObject.value(QLatin1String("text")).toObject();
    const QString text = jsonTextObject.value("*").toString().trimmed();
     ...
     */
    const QRegularExpression titleRegEx("<div.*?class=\"description.*?>.*?<a href=\"(.+?)\".*?>(.+?)</a>");
...
void WcpotdProvider::imageRequestFinished(KJob *_job)
{
    ...
    m_data->wallpaperImage = QImage::fromData(data);
