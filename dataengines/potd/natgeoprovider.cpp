...
void NatGeoProvider::pageRequestFinished(KJob *_job)
{
    ...
    for (int i = 0; i < lines.size(); i++) {
        ...
        if (match.hasMatch()) {
            m_data->wallpaperRemoteUrl = QUrl(match.captured(1));
        }
    }

    if (m_data->wallpaperRemoteUrl.isEmpty()) {
        Q_EMIT error(this);
        return;
    }
     ...
     */
    const QRegularExpression linkRegEx("<meta.*?property=\"og:url\" content=\"(.+?)\".*?>");
    const QRegularExpressionMatch linkMatch = linkRegEx.match(simplifiedData);
    if (linkMatch.hasMatch()) {
        m_data->wallpaperInfoUrl = QUrl(linkMatch.captured(1).trimmed());
    }
    ...
    if (authorMatch.hasMatch()) {
        m_data->wallpaperAuthor = QTextDocumentFragment::fromHtml(authorMatch.captured(1).remove("Photograph by").trimmed()).toPlainText();
    }
