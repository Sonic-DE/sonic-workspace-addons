...
void Kameleon::applyColor(QColor color)
{
    ...
    for (const QString &colorIndex : m_deviceRgbIndices) {
        ...
        colorBytesList[colorIndex.indexOf("r")] = QString::number(color.red());
        colorBytesList[colorIndex.indexOf("g")] = QString::number(color.green());
        colorBytesList[colorIndex.indexOf("b")] = QString::number(color.blue());
        colorStrs.append(colorBytesList.join(" "));
    }
