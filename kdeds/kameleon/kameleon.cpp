...
void Kameleon::findRgbLedDevices()
{
    ...
    for (const QString &ledDevice : ledDevices) {
        // Get RGB capability and save multicolor index like "grb" so we know later in what order to write the color bytes in
...
void Kameleon::loadLedColor()
{
    ...
    for (const QString &ledDevice : m_rgbLedDevices) {
        ...
        if (activeColor != m_activeColor) {
            ...
            Q_EMIT activeColorChanged(m_activeColor.name();
        }
    }
}
...
void Kameleon::activeColorChanged(QString colorName)
{
}
